...
class backpropAPLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, delta= 1.0, positive_label= 1, negative_label= 0):
        ...
        #print(f"Num pos: {labels_p.sum().item()} Num neg: {(~labels_p).sum().item()}")
...
class APLoss(nn.Module):
    def __init__(self, delta= 0.2, positive_label= 1, negative_label= 0):
...
class BatchedAPLoss(nn.Module):
    ...
    def forward(self, batched_scores, batched_assignments):
        ...
        ap_terms = 0
        for i in range(len(batched_scores)):
            scores_i = batched_scores[i]
            targets_i = batched_assignments[i]
            for cls in range(len(scores_i)):
                if torch.sum(targets_i[cls]) == 0:
                    continue
                cls_scores = scores_i[cls]      
                cls_labels = targets_i[cls].float()
                current_ap_loss = self.ap_loss(cls_scores, cls_labels)
                ap_loss = ap_loss + current_ap_loss
                ap_loss_cls_sum[cls] += current_ap_loss.detach()
                ap_loss_cls_cnt[cls] += 1
                ap_terms += 1
        ...
        ap_loss = ap_loss / max(ap_terms, 1)

    
        return {'loss_post': self.weight * ap_loss}


# class BatchedAPLoss(nn.Module):
#     def __init__(self, delta= 1.0, n_classes=3 , positive_label= 1, negative_label= 0):
#         super(BatchedAPLoss, self).__init__()
#         self.ap_loss = APLoss(delta=delta,  positive_label=positive_label, negative_label=negative_label)
#         self.delta          = delta
#         self.positive_label = positive_label
#         self.negative_label = negative_label
...
#     def set_delta(self, delta: float):
#         self.delta = float(delta)
#         self.ap_loss.set_delta(delta)

#     def forward(self, batched_scores, batched_assignment):
#         device = batched_scores[0][0].device
#         ap_loss = 0.0
#         ap_loss_cls_sum = torch.zeros(self.n_classes, device=device)
#         ap_loss_cls_cnt = torch.zeros(self.n_classes, device=device)
...
#         for i in range(len(batched_scores)):
#             scores_i = batched_scores[i]
#             targets_i = batched_assignment[i]
#             for cls in range(len(scores_i)):
#                 if torch.sum(targets_i[cls]) == 0:
#                     continue
#                 cls_scores = scores_i[cls]      
